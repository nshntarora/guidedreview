...
describe("ReviewSubmittedModal", () => {
  it("renders nothing when closed", () => {
    const { container } = render(
      <ReviewSubmittedModal open={false} event="APPROVE" commentCount={0} onExit={vi.fn()} />,
    );
    expect(container).toBeEmptyDOMElement();
  });

  it("shows the success title and check icon", () => {
    render(<ReviewSubmittedModal open event="APPROVE" commentCount={0} onExit={vi.fn()} />);

    expect(screen.getByRole("dialog", { name: "Review Submitted" })).toBeInTheDocument();
    expect(screen.getByTestId("review-submitted-icon")).toBeInTheDocument();
    expect(screen.getByTestId("review-submitted-modal")).toHaveAttribute("data-event", "APPROVE");
  });
  ...
  it("summarizes approval with a singular comment count", () => {
    render(<ReviewSubmittedModal open event="APPROVE" commentCount={1} onExit={vi.fn()} />);
    expect(screen.getByTestId("review-submitted-summary")).toHaveTextContent(
  ...
  it("summarizes a comment review with plural comments", () => {
    render(<ReviewSubmittedModal open event="COMMENT" commentCount={3} onExit={vi.fn()} />);
    expect(screen.getByTestId("review-submitted-summary")).toHaveTextContent(
  ...
  it("summarizes requested changes without comments", () => {
    render(<ReviewSubmittedModal open event="REQUEST_CHANGES" commentCount={0} onExit={vi.fn()} />);
    expect(screen.getByTestId("review-submitted-summary")).toHaveTextContent(
  ...
  it("summarizes requested changes with comments", () => {
    render(<ReviewSubmittedModal open event="REQUEST_CHANGES" commentCount={2} onExit={vi.fn()} />);
    expect(screen.getByTestId("review-submitted-summary")).toHaveTextContent(
  ...
  it("calls onExit from the primary button", () => {
    const onExit = vi.fn();
    render(<ReviewSubmittedModal open event="APPROVE" commentCount={0} onExit={onExit} />);
