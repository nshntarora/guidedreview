...
async function startAwaitingFlow(user: ReturnType<typeof userEvent.setup>) {
  ...
  render(<ConnectGitHubModal open onClose={vi.fn()} onAuthenticated={vi.fn()} />);
  await user.click(screen.getByTestId("connect-github-connect"));
  expect(await screen.findByTestId("connect-github-user-code")).toHaveTextContent("WDJB-MJHT");
}
...
describe("ConnectGitHubModal", () => {
  ...
  it("opens GitHub when Enter Code On GitHub is clicked", async () => {
    ...
    expect(openVerificationUriSpy).toHaveBeenCalledWith("https://github.com/login/device");
  });
  ...
  it("calls onAuthenticated after a successful poll", async () => {
    ...
    render(<ConnectGitHubModal open onClose={vi.fn()} onAuthenticated={onAuthenticated} />);
    await user.click(screen.getByTestId("connect-github-connect"));
  ...
  it("calls onClose when Cancel is clicked", async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    render(<ConnectGitHubModal open onClose={onClose} onAuthenticated={vi.fn()} />);
