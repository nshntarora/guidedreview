...
describe("submitPullRequestReview", () => {
  ...
  it("always fetches head sha and posts commit_id even without comments", async () => {
    ...
    expect(fetchMock.mock.calls[0][0]).toBe("https://api.github.com/repos/acme/widget/pulls/42");
  ...
  it("maps 404 on POST after successful head fetch", async () => {
    const fetchMock = mockHeadThen(jsonResponse({ message: "Not Found" }, 404));
    vi.stubGlobal("fetch", fetchMock);
      ...
      jsonResponse(
        {
          ...
          errors: [{ message: "line must be part of the diff" }, { message: "path is invalid" }],
          documentation_url:
  ...
  it("fails when head sha is missing", async () => {
    ...
    const result = await submitPullRequestReview({
      ...
      body: "x",
      event: "COMMENT",
      comments: [{ path: "a.ts", body: "c", side: "RIGHT", line: 1 }],
    });
