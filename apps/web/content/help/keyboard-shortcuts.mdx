...
The Guided Review sidebar always lists the same shortcuts as this page. Most chords work in both the [Chrome extension](/docs/first-review) and the [CLI](/docs/local-review); [Local review](#local-review) adds CLI-only keys. Submit chords apply to the extension only.
...
The [CLI](/docs/local-review) uses the same navigation, diff view, search, and comment-mode keys as the extension. Submit (`⌘/Ctrl` + `Enter` for a GitHub review) does not apply — use **Generate Prompt** from the UI instead. Extra in the local UI:
...
| `⌘/Ctrl` + `E` | Open the Chrome Web Store listing — also [Install Guided Review](/docs/install)                 |
